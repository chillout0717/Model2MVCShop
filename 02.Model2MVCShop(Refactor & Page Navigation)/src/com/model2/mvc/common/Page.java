...
public class Page {
	...
	public Page( int currentPage, int totalCount,	int pageUnit, int pageSize ) {
		this.totalCount = totalCount; // 총 게시물의 수 
		this.pageUnit = pageUnit; // 하단 페이지 번호 화면에 보여지는 수 
		this.pageSize = pageSize; //한패이지당 보여지는 게시물수 
		
		this.maxPage = (pageSize == 0) ? totalCount :  (totalCount-1)/pageSize +1;
		//하단에 최대 페이지 수를 정하기 위한 로직 
		this.currentPage = ( currentPage > maxPage) ? maxPage : currentPage;
		//maxpage 이상으로 페이지가 넘어가지 않게 커런트 페이즈를 출력하기 위한 로직
		this.beginUnitPage = ( (currentPage-1) / pageUnit ) * pageUnit +1 ;
		// 하단의 페이지 최소 수를 알려준는데 쓴다 
		if( maxPage <= pageUnit ){
